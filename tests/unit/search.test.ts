...
// the real committed content, loaded through src/content/loaders.ts.
...
describe('search', () => {
  ...
  it('builds the index from the real committed content (logs size and timing)', async () => {
    const start = Date.now();
    const sections = await loadAllSections();
    const index = await getSearchIndex();
    const elapsedMs = Date.now() - start;
    const ruleDocCount = sections.reduce((total, section) => total + section.rules.length, 0);
    const sectionDocCount = sections.filter((section) => section.rules.length === 0).length;
    console.log(
